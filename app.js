...
function visualScene(kind){
 const status='<div class="viz-status" data-widget-status><span></span><b>Ready</b></div>';
 if(kind==="density") return '<div class="viz density-viz" data-widget="density">'+status+'<div class="mass-block" data-role="block">100 g</div><div class="viz-readout"><span>Mass <b>100 g</b></span><span>Volume <b data-role="volume">50 cm³</b></span><span>Density <b data-role="density">2.00 g/cm³</b></span></div><label class="viz-slider-label">Volume <input type="range" min="20" max="100" value="50" data-control="volume"></label><div class="viz-caption" data-role="caption">Keep mass fixed. Reduce volume and density increases.</div></div>';
 if(kind==="displacement") return '<div class="viz displacement-viz" data-widget="displacement">'+status+'<div class="displacement-stage"><div class="cylinder"><div class="graduations">'+Array.from({length:7},(_,i)=>'<i style="--g:'+i+'"></i>').join("")+'</div><div class="water" data-role="water"></div><div class="object-drop" data-role="object">metal</div><span class="level initial">40</span><span class="level final">58</span></div><div class="disp-equation"><span>V<sub>object</sub></span><b>=</b><span>58 mL</span><b>−</b><span>40 mL</span><b>=</b><strong>18 mL</strong></div></div><button class="mini-btn" data-action="drop">Drop object</button><div class="viz-caption" data-role="caption">Press Drop object. The rise in water level equals object volume.</div></div>';
 if(kind==="sigfig") return '<div class="viz sigfig-viz" data-widget="sigfig">'+status+'<div class="sig-number" data-role="number"><span class="digit ghost">0</span><span class="decimal">.</span><span class="digit ghost">0</span><span class="digit ghost">0</span><span class="digit sig">4</span><span class="digit sig">5</span><span class="digit sig">6</span><span class="digit sig">0</span><span class="digit sig">0</span></div><div class="sig-legend"><span><i class="placeholder-dot"></i>placeholder</span><span><i class="sig-dot"></i>significant</span><strong data-role="count">0 / 5 counted</strong></div><div class="viz-controls"><button class="mini-btn" data-action="scan">Scan digits</button><button class="mini-btn" data-action="reset">Reset</button></div><div class="viz-caption" data-role="caption">Leading zeros position the decimal and do not count.</div></div>';
 if(kind==="isotope") return '<div class="viz isotope-viz" data-widget="isotope">'+status+'<div class="isotope-scale"><span>35 amu</span><i data-role="needle"></i><span>37 amu</span></div><label class="viz-slider-label">Heavy isotope abundance <input type="range" min="0" max="100" value="24" data-control="abundance"></label><div class="viz-readout"><span>Heavy isotope <b data-role="pct">24%</b></span><span>Average mass <b data-role="avg">35.48 amu</b></span></div><div class="viz-caption" data-role="caption">The weighted average moves toward the more abundant isotope.</div></div>';
 if(kind==="wave") return '<div class="viz wave-viz" data-widget="wave">'+status+'<svg viewBox="0 0 520 130" preserveAspectRatio="none"><path data-role="wave" d="" fill="none" stroke="currentColor" stroke-width="4"/></svg><label class="viz-slider-label">Relative frequency <input type="range" min="1" max="8" value="3" data-control="frequency"></label><div class="viz-readout"><span>Frequency <b data-role="freq">3×</b></span><span>Relative wavelength <b data-role="lambda">0.33×</b></span></div><div class="viz-caption" data-role="caption">Frequency up means wavelength down when wave speed is fixed.</div></div>';
 if(kind==="molarity") return '<div class="viz molarity-viz" data-widget="molarity">'+status+'<div class="beaker"><div class="solution" data-role="solution"></div><div class="particles" data-role="particles"></div></div><div class="dual-control"><label>Solute amount<input type="range" min="1" max="16" value="6" data-control="solute"></label><label>Solution volume<input type="range" min="40" max="100" value="80" data-control="volume"></label></div><div class="viz-readout"><span>Relative concentration <b data-role="concentration">0.75</b></span></div><div class="viz-caption" data-role="caption">More solute or less total solution volume raises concentration.</div></div>';
 if(kind==="gas") return '<div class="viz gas-viz" data-widget="gas">'+status+'<div class="gas-layout"><div class="piston"><div class="piston-handle"></div><div class="piston-head" data-role="piston"></div><div class="gas-space" data-role="space">'+Array.from({length:18},(_,i)=>'<i style="--x:'+((i*37)%88+6)+'%;--y:'+((i*53)%78+10)+'%;--d:'+(0.75+(i%5)*.12)+'s"></i>').join("")+'</div></div><div class="gas-meter"><span>Relative volume</span><strong data-role="volume">70%</strong><span>Relative pressure</span><strong data-role="pressure">1.43×</strong></div></div><label class="viz-slider-label">Piston position <input type="range" min="25" max="85" value="70" data-control="volume"></label><div class="viz-caption" data-role="caption">Boyle relationship: at constant temperature, lower volume means higher pressure.</div></div>';
 if(kind==="heat") return '<div class="viz heat-viz" data-widget="heat">'+status+'<div class="thermal-pair"><div class="thermal hot"><span>Object A</span><b data-role="hot-temp">80°C</b><div class="thermo-bar"><i data-role="hot-bar"></i></div></div><div class="heat-flow"><span data-role="arrow">→</span><small>q</small></div><div class="thermal cold"><span>Object B</span><b data-role="cold-temp">20°C</b><div class="thermo-bar"><i data-role="cold-bar"></i></div></div></div><label class="viz-slider-label">Energy-transfer progress <input type="range" min="0" max="100" value="0" data-control="progress"></label><div class="viz-caption" data-role="caption">Thermal energy moves from hotter matter to colder matter.</div></div>';
 if(kind==="equilibrium") return '<div class="viz equilibrium-viz" data-widget="equilibrium">'+status+'<div class="eq-box"><b>Reactants</b><strong data-role="react-count">80</strong><div class="eq-pop" data-role="react-pop"></div></div><div class="eq-center"><div class="eq-arrows"><span>→</span><span>←</span></div><div class="rate-badge"><small>forward rate</small><b data-role="forward">1.0</b><small>reverse rate</small><b data-role="reverse">1.0</b></div></div><div class="eq-box"><b>Products</b><strong data-role="prod-count">20</strong><div class="eq-pop" data-role="prod-pop"></div></div><label class="viz-slider-label eq-slider">Composition <input type="range" min="5" max="95" value="20" data-control="composition"></label><div class="viz-caption" data-role="caption">Dynamic equilibrium means equal rates, not necessarily equal amounts.</div></div>';
 if(kind==="ph") return '<div class="viz ph-viz" data-widget="ph">'+status+'<div class="ph-state"><span data-role="class">Neutral</span><strong data-role="value">pH 7</strong><small data-role="h">[H⁺] ≈ 1 × 10⁻⁷ M</small></div><div class="ph-scale" data-role="scale">'+Array.from({length:15},(_,i)=>'<span data-ph="'+i+'">'+i+'</span>').join("")+'<i data-role="marker"></i></div><label class="viz-slider-label">pH <input type="range" min="0" max="14" value="7" data-control="ph"></label><div class="ph-caption-row"><span>acidic</span><span>neutral</span><span>basic</span></div><div class="viz-caption" data-role="caption">One pH unit corresponds to a tenfold change in hydrogen-ion concentration.</div></div>';
 if(kind==="kinetics") return '<div class="viz kinetics-viz" data-widget="kinetics">'+status+'<div class="kinetics-chart"><div class="axis-label y">Potential energy</div><svg viewBox="0 0 520 190"><path data-role="uncat" d="M10 155 C110 150 130 25 230 25 S340 145 510 110" fill="none" stroke="currentColor" stroke-width="4"/><path data-role="cat" d="M10 155 C120 150 145 80 230 80 S350 145 510 110" fill="none" stroke="currentColor" stroke-width="4" opacity=".35"/><line x1="10" y1="155" x2="510" y2="155" class="chart-base"/></svg><div class="axis-label x">Reaction progress</div></div><div class="kinetics-readout"><span>Uncatalyzed barrier <b>high</b></span><span>Catalyzed barrier <b data-role="barrier">high</b></span></div><label class="viz-slider-label">Catalyst effect <input type="range" min="0" max="100" value="0" data-control="catalyst"></label><div class="viz-caption" data-role="caption">A catalyst lowers activation energy without changing reactant/product energies.</div></div>';
 if(kind==="phase") return '<div class="viz phase-viz" data-widget="phase">'+status+'<div class="phase-chart"><div class="axis-label y">Temperature</div><svg viewBox="0 0 520 200"><path d="M20 175 L120 105 L225 105 L325 45 L455 45 L505 15" fill="none" stroke="currentColor" stroke-width="5"/><circle data-role="dot" cx="20" cy="175" r="8" fill="currentColor"/></svg><div class="axis-label x">Heat added</div></div><div class="phase-stages"><span data-stage="0">solid warms</span><span data-stage="1">melting</span><span data-stage="2">liquid warms</span><span data-stage="3">boiling</span><span data-stage="4">gas warms</span></div><label class="viz-slider-label">Heat added <input type="range" min="0" max="100" value="0" data-control="heat"></label><div class="viz-caption" data-role="caption">Add heat and follow the sample through each region.</div></div>';
 return '<div class="viz" data-widget="unknown">'+status+'<div class="viz-caption">Concept model</div></div>'
}
...
function bindVisualLab(){
 ...
 const setStatus=(card,state,msg)=>{const s=q(card,"[data-widget-status]");if(!s)return;s.className="viz-status "+state;q(s,"b").textContent=msg};
 const safe=(card,fn)=>{try{fn();setStatus(card,"ok","Interactive")}catch(err){setStatus(card,"error","Error");console.error("Visual Lab widget failed:",card.dataset.widget,err)}};
 ...
 cards.forEach(card=>safe(card,()=>{
   ...
   if(kind==="density"){
     const control=q(card,'[data-control="volume"]'),update=()=>{const v=+control.value;q(card,'[data-role="volume"]').textContent=v+" cm³";q(card,'[data-role="density"]').textContent=(100/v).toFixed(2)+" g/cm³";q(card,'[data-role="block"]').style.transform="scaleX("+Math.max(.48,1.25-v/160)+")";q(card,'[data-role="caption"]').textContent=v<50?"Less volume with the same mass means greater density.":"More volume with the same mass means lower density."};control.addEventListener("input",update);update()
   } else if(kind==="displacement"){
     const btn=q(card,'[data-action="drop"]');btn.addEventListener("click",()=>{const active=!card.classList.contains("active");card.classList.toggle("active",active);btn.textContent=active?"Reset experiment":"Drop object";q(card,'[data-role="caption"]').textContent=active?"Final 58 mL − initial 40 mL = 18 mL. The water rise is the object's volume.":"Press Drop object. The rise in water level equals object volume."})
   } else if(kind==="sigfig"){
     const scan=q(card,'[data-action="scan"]'),reset=q(card,'[data-action="reset"]'),num=q(card,'[data-role="number"]'),digits=qa(card,".sig");
     scan.addEventListener("click",()=>{num.classList.remove("scanning");void num.offsetWidth;num.classList.add("scanning");q(card,'[data-role="count"]').textContent="0 / 5 counted";digits.forEach((d,i)=>setTimeout(()=>{if(!document.body.contains(card))return;q(card,'[data-role="count"]').textContent=(i+1)+" / 5 counted";if(i===4)q(card,'[data-role="caption"]').textContent="4, 5, 6 and both trailing decimal zeros are significant."},220*(i+1)))});
     reset.addEventListener("click",()=>{num.classList.remove("scanning");q(card,'[data-role="count"]').textContent="0 / 5 counted";q(card,'[data-role="caption"]').textContent="Leading zeros position the decimal and do not count."})
   } else if(kind==="isotope"){
     const control=q(card,'[data-control="abundance"]'),update=()=>{const p=+control.value;q(card,'[data-role="pct"]').textContent=p+"%";q(card,'[data-role="avg"]').textContent=(35*(1-p/100)+37*(p/100)).toFixed(2)+" amu";q(card,'[data-role="needle"]').style.left=p+"%"};control.addEventListener("input",update);update()
   } else if(kind==="wave"){
     const control=q(card,'[data-control="frequency"]'),update=()=>{const n=+control.value;let d="M0 65";for(let x=0;x<=520;x+=4){const y=65+38*Math.sin((x/520)*Math.PI*2*n);d+=" L"+x+" "+y}q(card,'[data-role="wave"]').setAttribute("d",d);q(card,'[data-role="freq"]').textContent=n+"×";q(card,'[data-role="lambda"]').textContent=(1/n).toFixed(2)+"×";q(card,'[data-role="caption"]').textContent="At fixed wave speed, frequency × wavelength stays constant."};control.addEventListener("input",update);update()
   } else if(kind==="molarity"){
     const solute=q(card,'[data-control="solute"]'),volume=q(card,'[data-control="volume"]'),update=()=>{const n=+solute.value,v=+volume.value;q(card,'[data-role="solution"]').style.height=v+"%";q(card,'[data-role="particles"]').innerHTML=Array.from({length:n},(_,i)=>'<i style="--x:'+((i*41)%78+10)+'%;--y:'+((i*29)%Math.max(30,v-10)+8)+'%"></i>').join("");q(card,'[data-role="concentration"]').textContent=(n/v*10).toFixed(2)};solute.addEventListener("input",update);volume.addEventListener("input",update);update()
   } else if(kind==="gas"){
     const control=q(card,'[data-control="volume"]'),update=()=>{const h=+control.value,p=100/h;q(card,'[data-role="space"]').style.height=h+"%";q(card,'[data-role="piston"]').style.top=(100-h)+"%";q(card,'[data-role="volume"]').textContent=h+"%";q(card,'[data-role="pressure"]').textContent=p.toFixed(2)+"×";q(card,'[data-role="caption"]').textContent=h<45?"Compression increases collision frequency with the walls, so pressure rises.":"Expansion lowers relative pressure at constant temperature."};control.addEventListener("input",update);update()
   } else if(kind==="heat"){
     const control=q(card,'[data-control="progress"]'),update=()=>{const x=+control.value/100,hot=80-30*x,cold=20+30*x;q(card,'[data-role="hot-temp"]').textContent=hot.toFixed(0)+"°C";q(card,'[data-role="cold-temp"]').textContent=cold.toFixed(0)+"°C";q(card,'[data-role="hot-bar"]').style.width=(hot/80*100)+"%";q(card,'[data-role="cold-bar"]').style.width=(cold/80*100)+"%";q(card,'[data-role="arrow"]').style.opacity=String(Math.max(.15,1-x));q(card,'[data-role="caption"]').textContent=x>.96?"Thermal equilibrium: the temperatures have converged.":"Heat flows from A to B. Current temperature gap: "+Math.round(hot-cold)+"°C."};control.addEventListener("input",update);update()
   } else if(kind==="equilibrium"){
     const control=q(card,'[data-control="composition"]'),update=()=>{const p=+control.value,r=100-p;q(card,'[data-role="prod-count"]').textContent=p;q(card,'[data-role="react-count"]').textContent=r;particles(q(card,'[data-role="react-pop"]'),r,"react");particles(q(card,'[data-role="prod-pop"]'),p,"prod");const rf=r/80,rr=p/20;q(card,'[data-role="forward"]').textContent=rf.toFixed(1);q(card,'[data-role="reverse"]').textContent=rr.toFixed(1);q(card,'[data-role="caption"]').textContent=Math.abs(rf-rr)<.15?"Forward and reverse rates are approximately equal: dynamic equilibrium.":"Rates differ, so the system tends to shift until the rates match."};control.addEventListener("input",update);update()
   } else if(kind==="ph"){
     const control=q(card,'[data-control="ph"]'),update=()=>{const v=+control.value,cls=v<7?"Acidic":v>7?"Basic":"Neutral";q(card,'[data-role="value"]').textContent="pH "+v;q(card,'[data-role="class"]').textContent=cls;q(card,'[data-role="h"]').textContent="[H⁺] ≈ 1 × 10⁻"+v+" M";q(card,'[data-role="marker"]').style.left=(v/14*100)+"%";qa(card,"[data-ph]").forEach(s=>s.classList.toggle("active",+s.dataset.ph===v));q(card,'[data-role="caption"]').textContent=v===7?"Neutral reference point.":"Each pH step changes [H⁺] tenfold."};control.addEventListener("input",update);update()
   } else if(kind==="kinetics"){
     const control=q(card,'[data-control="catalyst"]'),update=()=>{const x=+control.value/100,peak=25+85*x;q(card,'[data-role="cat"]').setAttribute("d","M10 155 C120 150 145 "+peak+" 230 "+peak+" S350 145 510 110");q(card,'[data-role="cat"]').style.opacity=String(.3+.7*x);q(card,'[data-role="barrier"]').textContent=x<.25?"high":x<.65?"medium":"low";q(card,'[data-role="caption"]').textContent=x<.1?"Increase catalyst effect to reveal the alternate pathway.":"Catalyst lowers activation energy while endpoints remain unchanged."};control.addEventListener("input",update);update()
   } else if(kind==="phase"){
     const control=q(card,'[data-control="heat"]'),pts=[[20,175],[120,105],[225,105],[325,45],[455,45],[505,15]],segs=[18,22,20,22,18],update=()=>{let p=+control.value,acc=0,idx=0;for(let i=0;i<segs.length;i++){if(p<=acc+segs[i]){idx=i;break}acc+=segs[i];idx=i}const t=Math.max(0,Math.min(1,(p-acc)/segs[idx])),a=pts[idx],b=pts[idx+1],dot=q(card,'[data-role="dot"]');dot.setAttribute("cx",a[0]+(b[0]-a[0])*t);dot.setAttribute("cy",a[1]+(b[1]-a[1])*t);qa(card,"[data-stage]").forEach(s=>s.classList.toggle("active",+s.dataset.stage===idx));const labels=["solid warming","melting","liquid warming","boiling","gas warming"];q(card,'[data-role="caption"]').textContent="Current stage: "+labels[idx]+". "+((idx===1||idx===3)?"Energy changes phase while temperature stays nearly constant.":"Temperature rises as particle kinetic energy increases.")};control.addEventListener("input",update);update()
   }
 }));

 // Browser-side validation: every declared widget must initialize successfully.
 const report=cards.map(card=>({name:card.dataset.widget,status:q(card,"[data-widget-status] b")?.textContent||"Unknown"}));
 const failed=report.filter(x=>x.status==="Error");
 if(failed.length)console.error("Visual Lab validation failures:",failed);else console.info("Visual Lab validation passed:",report.map(x=>x.name).join(", "));
}

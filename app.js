...
function visualScene(kind){
 ...
 if(kind==="sigfig") return '<div class="viz sigfig-viz"><div class="sig-number"><span class="ghost">0.00</span><span class="sig">45600</span></div><div class="viz-caption">0.0045600 has 5 significant figures.</div></div>';
 ...
 if(kind==="heat") return '<div class="viz heat-viz"><div class="thermal hot">HOT</div><div class="heat-arrow">→</div><div class="thermal cold">COLD</div><div class="viz-caption">Thermal energy moves from hotter to colder matter.</div></div>';
 if(kind==="equilibrium") return '<div class="viz equilibrium-viz"><div class="eq-box"><b>Reactants</b></div><div class="eq-arrows"><span>→</span><span>←</span></div><div class="eq-box"><b>Products</b></div><div class="viz-caption">Both directions continue while rates become equal.</div></div>';
 ...
 if(kind==="kinetics") return '<div class="viz kinetics-viz"><svg viewBox="0 0 520 190"><path d="M10 155 C110 150 130 25 230 25 S340 145 510 110" fill="none" stroke="currentColor" stroke-width="4"/><path d="M10 155 C120 150 145 80 230 80 S350 145 510 110" fill="none" stroke="currentColor" stroke-width="4" opacity=".35"/></svg><div class="viz-caption">Catalyst lowers activation energy without changing ΔG.</div></div>';
 if(kind==="phase") return '<div class="viz phase-viz"><svg viewBox="0 0 520 200"><path d="M20 175 L120 105 L225 105 L325 45 L455 45 L505 15" fill="none" stroke="currentColor" stroke-width="5"/></svg><div class="viz-caption">Plateaus represent phase changes while energy still enters.</div></div>';
...
function bindVisualLab(){
 const density=$('[data-viz-control="density"]');if(density)density.oninput=e=>{const v=+e.target.value;$("#densityVol").textContent=v+" cm³";$("#densityOut").textContent=(100/v).toFixed(2)+" g/cm³";$("#densityBlock").style.transform="scaleX("+Math.max(.48,1.25-v/160)+")"};
 $('[data-animate="displacement"]').forEach(b=>b.onclick=()=>b.closest(".displacement-viz")?.classList.toggle("active"));
 const iso=$('[data-viz-control="isotope"]');if(iso)iso.oninput=e=>{const p=+e.target.value;$("#isoPct").textContent=p+"%";$("#isoAvg").textContent=(35*(1-p/100)+37*(p/100)).toFixed(2)+" amu";$("#isoNeedle").style.left=p+"%"};
 const wave=$('[data-viz-control="wave"]');if(wave){const draw=()=>{const n=+wave.value;let d="M0 65";for(let x=0;x<=520;x+=4){const y=65+38*Math.sin((x/520)*Math.PI*2*n);d+=" L"+x+" "+y}$("#wavePath").setAttribute("d",d);$("#waveCaption").textContent="Relative frequency "+n+"×. More cycles in the same distance means shorter wavelength."};wave.oninput=draw;draw()}
 const solute=$('[data-viz-control="solute"]'),vol=$('[data-viz-control="volume"]');if(solute&&vol){const update=()=>{const n=+solute.value,v=+vol.value;$("#molSolution").style.height=v+"%";$("#molParticles").innerHTML=Array.from({length:n},(_,i)=>'<i style="--x:'+((i*41)%78+10)+'%;--y:'+((i*29)%Math.max(30,v-10)+8)+'%"></i>').join("");$("#molCaption").textContent="Relative concentration: "+(n/v*10).toFixed(2)+" units"};solute.oninput=vol.oninput=update;update()}
 const gas=$('[data-viz-control="gas"]');if(gas)gas.oninput=e=>{const h=+e.target.value;$("#gasSpace").style.height=h+"%";$("#gasCaption").textContent=h<45?"Smaller volume → more frequent wall collisions → higher pressure.":"Larger volume → fewer wall collisions per area → lower pressure."};
 const ph=$('[data-viz-control="ph"]');if(ph)ph.oninput=e=>{const v=+e.target.value;$("#phCaption").textContent="pH "+v+" → [H⁺] ≈ 1 × 10⁻"+v+" M"};
}
